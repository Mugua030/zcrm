...
struct UserStat {
    ...
    name: String,
    gender: Gender,
    #[dummy(faker = "DateTimeBetween(before(356*5), before(90))")]
...
async fn main() -> Result<()> {
    let db_pool = PgPool::connect("postgres://postgres:123456@localhost:5432/stats").await?;

    for i in 1..=3 {
        let users: HashSet<_> = (0..3).map(|_| Faker.fake::<UserStat>()).collect();

        let start = Instant::now();
        //println!("user: {:?}", user);
        raw_insert(users, &db_pool).await?;
